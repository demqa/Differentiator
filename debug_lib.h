...
#define PRINT_C(char)  do{ fprintf(__OUT, "[%s:%lu] %s = %c\n", __PRETTY_FUNCTION__, __LINE__, #char, char);        }while(0);

#define PRINT_S(str)   do{ fprintf(__OUT, "[%s:%lu] %s = %s\n", __PRETTY_FUNCTION__, __LINE__, #str, str);          }while(0);

#define PRINT(str)     do{ fprintf(__OUT, "%s\n", #str); }while(0);

#define PRINT_UL(num)  do{ fprintf(__OUT, "[%s:%lu] %s = %lu\n", __PRETTY_FUNCTION__, __LINE__, #num, num);         }while(0);
       
#define PRINT_D(num)   do{ fprintf(__OUT, "[%s:%lu] %s = %d\n", __PRETTY_FUNCTION__, __LINE__, #num, num);          }while(0);
       
#define PRINT_X(num)   do{ fprintf(__OUT, "[%s:%lu] %s = %x\n", __PRETTY_FUNCTION__, __LINE__, #num, num);          }while(0);
